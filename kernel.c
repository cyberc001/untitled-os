...
void kernel_main(void)
{
	...
	bios_vga_printf("blockgroup entry #0, block bitmap address: %lu\n", bgrp_table.groups[0].block_bitmap_addr);
	...
	{
		bios_vga_printf("%lu %s\n", ie1_it.cur.inode_num, ie1_it.cur.name);
	}
	bios_vga_printf("%lu %s\n", ie1_it.cur.inode_num, ie1_it.cur.name);
